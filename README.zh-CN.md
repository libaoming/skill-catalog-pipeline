...
完整 17 条见 **[docs/engineering-notes.zh-CN.md](docs/engineering-notes.zh-CN.md)**，这里挑最容易踩、代价最大的：
...
| 文档 | 内容 |
|---|---|
| **[docs/architecture.zh-CN.md](docs/architecture.zh-CN.md)** | 脱敏完整方案：五层管线逐层展开 + 总体架构图 + 建表 / 索引 / 检索 SQL |
| **[docs/engineering-notes.zh-CN.md](docs/engineering-notes.zh-CN.md)** | ⭐ 工程结晶：17 条踩坑复盘（现象 → 根因 → 结论），本仓库最高价值 |
| **[docs/field-dictionary.zh-CN.md](docs/field-dictionary.zh-CN.md)** | 字段字典：抓取 → 清洗 → 落库三套口径，及字段演变小结 |
| **[src/](src/)** | 脱敏参考实现：`fetch/` `normalize/` `storage/` `retrieval/` `sync/` 五层脚本 + SQL，见 [src/README.zh-CN.md](src/README.zh-CN.md) |
